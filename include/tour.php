...
<?php
$result=$conn->query("select * from tour_type");?>
        <thead>
              <tr>
                <th hidden>#</th>
                <th>Action</th>
...
<script>
    $(document).ready(function(){
        $('#frmTour').on('submit', function(e){
            e.preventDefault()
            $.ajax({
                url:'class/class.php?action=add_tour',
                method:'POST',
        ...
        $('.edit-tour').on('click', function(){
            ...
            document.getElementById('tour_name').value=$(this).closest('tr').find('td:eq(2)').text().trim()
        })
